...
const EMAIL_TYPES = [
  { id: "leave",       label: "🏖️ Leave Application",    hint: "Sick leave, casual leave, medical leave" },
  ...
  { id: "complaint",   label: "⚠️ Complaint",             hint: "Complaint to company, bank, service" },
...
export default function AIEmailPage() {
  ...
  const generate = async () => {
    ...
    const prompt = `Write a professional ${type.label.replace(/[^\w\s]/gi,"").trim()} email.
${toName ? `To: ${toName}` : ""}
...
- Proper email format with greeting, body and closing
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1000,
          system: "You are an expert email writer. Write professional emails for Indian workplace context. Always include Subject line, proper greeting, body and signature. Output only the email, nothing else.",
          messages: [{ role: "user", content: prompt }],
        }),
      });
      const data = await res.json();
      setOutput(data.content?.find((b: {type:string}) => b.type==="text")?.text || "Error generating email.");
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-black/40 backdrop-blur-sm border border-zinc-700/60 rounded-3xl p-8 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="block text-gray-400 text-xs mb-1">Recipient Name / Company</label>
            ...
            <div>
              ...
              <textarea rows={4} value={details} onChange={e=>setDetails(e.target.value)}
                placeholder={
                  emailType==="leave" ? "e.g. I need leave on Monday and Tuesday for fever. I am on project XYZ." :
                  emailType==="job_apply" ? "e.g. Applying for Software Engineer. 2 years React experience. B.Tech from Mumbai University 2022." :
                  emailType==="complaint" ? "e.g. My internet connection at account 12345 is not working since 3 days despite multiple calls." :
                  "Describe the key details, reason, or any specific information to include..."
                }
