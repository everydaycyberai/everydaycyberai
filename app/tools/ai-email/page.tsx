  ...
  const generate = async () => {
    ...
    try {
      const res = await fetch("/api/ai", {
        ...
        body: JSON.stringify({
          systemPrompt: "You are an expert professional email writer for Indian workplace context. Always start with 'Subject: ...' on the first line, then a blank line, then the email. Output only the email, no extra commentary.",
          userPrompt: `Write a professional ${type.label.replace(/[^\w\s]/gi,"").trim()} email.
${toName ? `To: ${toName}` : ""}
${fromName ? `From: ${fromName}` : ""}
Details: ${details}

Requirements: professional tone, proper format, concise, Indian workplace context.`,
        }),
      });
      ...
      <main className="px-6 py-12 pb-24 relative z-10">
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-10">
            ...
            <p className="text-gray-400">Generate professional emails instantly — leave application, job apply, complaints and more. Free AI email writer India.</p>
          </div>
          ...
          <div className="bg-black/40 backdrop-blur-sm border border-zinc-700/60 rounded-3xl p-8 space-y-5">
            ...
            <div>
              ...
              <textarea rows={4} value={details} onChange={e => setDetails(e.target.value)}
                placeholder={
                  emailType==="leave" ? "e.g. Need leave on Monday and Tuesday for fever. Working on Project XYZ." :
