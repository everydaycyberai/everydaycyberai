...
type Options = { uppercase: boolean; lowercase: boolean; numbers: boolean; symbols: boolean };
...
function getStrength(pwd: string) {
  ...
  if (score <= 5) return { label: "Strong",      color: "bg-green-400",  text: "text-green-400",  width: "w-4/5"  };
  return               { label: "Very Strong",   color: "bg-cyan-400",   text: "text-cyan-400",   width: "w-full" };
}
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="bg-black/40 backdrop-blur-sm border border-zinc-700/60 rounded-3xl p-8 space-y-8">
                ...
                {([
                  ...
                  <button key={key} onClick={() => toggle(key)}
                    className={`flex items-center justify-between px-4 py-3 rounded-xl border transition duration-200 ${
                      opts[key] ? "bg-cyan-500/10 border-cyan-500/50 text-white" : "bg-black border-zinc-700 text-gray-500"}`}>
            ...
            <div className="space-y-3">
              {passwords.map((pwd, idx) => (
                <div key={idx} className="flex items-center gap-3 bg-black/80 border border-zinc-700 rounded-2xl px-5 py-4">
                  <span className="flex-1 font-mono text-lg break-all text-white">{pwd}</span>
                  <button onClick={() => copy(pwd, idx)}
                    className={`shrink-0 px-4 py-2 rounded-xl text-sm font-semibold transition duration-200 ${
                      copied === idx ? "bg-green-500/20 border border-green-500/40 text-green-400" : "bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 hover:bg-cyan-500/20"}`}>
          ...
          <div className="mt-8 bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6">
            ...
            <ul className="space-y-2 text-gray-400 text-sm">
              ...
              <li>✓ Passwords 16+ characters with all types are hardest to crack</li>
            </ul>
          </div>
        </div>
      </main>
